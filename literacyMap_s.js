    ...
    function render(selection) {
      selection.each(function() {
        d3.select(this).html("<form>"+
                             "<input type='radio' name='gender' value='TotalSeperated' checked> ALL<br>"+
                             "<input type='radio' name='gender' value='FemaleSeperated'> FEMALE<br>"+
                             "<input type='radio' name='gender' value='MaleSeperated'> MALE"+
                             "</form>");
      });
    } // render
return render;
...
function colorCode(data, filter) {
    var color = d3.scale.threshold()
                  .domain([0.12, 0.32, 0.52, 0.72, 0.92, 1.12, 1.32])
