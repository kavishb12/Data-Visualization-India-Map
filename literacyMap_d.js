    ...
    function render(selection) {
      selection.each(function() {
        d3.select(this).html("<form>"+
                             "<input type='radio' name='gender' value='TotalDivorced' checked> ALL<br>"+
                             "<input type='radio' name='gender' value='FemaleDivorced'> FEMALE<br>"+
                             "<input type='radio' name='gender' value='MaleDivorced'> MALE"+
                             "</form>");
      });
    } // render
return render;
...
function colorCode(data, filter) {
    var color = d3.scale.threshold()
                  .domain([0.05, 0.15, 0.25, 0.35, 0.45, 0.55, 0.65])
